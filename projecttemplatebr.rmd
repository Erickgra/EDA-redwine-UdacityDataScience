...
exceção de residial.sugar, alcohol, citric.acid e total.sufur.dioxide que parecem 
ter uma cauda mais longa a direita. Também identifiquei a presença de outliers em praticamente todas as variáveis.
...
entre as variáveis sulphates e quality. Assim, podemos analisar adistribuição das observações entre as categorias de Avaliação. E podemos observar que a variável
sulphates muda de acordo com as notas de qualidade aummentam.
...
podemos construir uma visão consolidada entre elas. E verificamos o comportamento
de cada uma delas com o aumento das notas de qualidade, onde alcohol tem uma
variação positiva, volatile.acidity negativa e sulphates segue uma distribuição
normal.
...
constatar um comportamento dependente, devido a correlação com quality ser baixa, 
uma investigação mais aprofundada é descartada.
...
Ao analisar a matriz de correlação observei que as três maiores correlações estão 
entre  a variavel qualidade são com: alcohol, volatile.acidity e sulphates. Sendo 
que a correlação mais forte é com alcohol (0.476) e tem uma tendência positiva. 
Já com relação a variável volatile.acidity (-0.391) a tendência é negativa. E com 
a variável sulphates (0.251) a corelação é menor e positiva. No caso do gráfico 
entre sulphates e quality optei em representar a variável sulphates na escala 
logaritmica (log10), fazendo com que a distribuição aproximasse-se de um padrão 
normal.
...
valor de correlação (0.668) do par fixed.acidity e density, mas por elas já serem relacionadas por natureza optei por não considerá-las como um relacionamento 
interessante.
...
fundamento não foi evidenciado nenhum comportamento distinto no conjunto de dados.
...
Mais uma investigação realizada foi da relação entre volatile.acidity e citric.acid,
com a identificação do grau de avaliação no mesmo gráfico. Nesse caso também não foi evidenciado nenhum comportamento distinto no conjunto de dados.
...
Iniciei investigando o relacionamento entre as variáveis alcohol e volatile.acidity 
por terem o maior coeficiente de correlação com quality.E na representação gráfica 
é possível identificar a segmentação dos vinhos por teor alcoólico: vinhos com uma 
...
Ao contrário de minhas suspeitas iniciais a variável residual.sugar não apresentou 
uma correlação que justificasse uma investigação aprofundada e acabou sendo 
descartada.
...
Infelizmente não foi possível explorar a criação de modelos, mas essa possibilidade 
existe para conjunto de dados, tanto pelo tamanho do conjunto de amostras, quanto
pelos relacionamentos entre as variaveis.
...
apresenta estes registros num histograma, que é segmentado pela variavel avaliação, 
que nada mais é do que um categorização da nota de qualidade. Podemos verificar
que a maior parte dos nossos registros pertencem a avaliação média.
...
Pela matriz de correlação é possível identificarmos os relacionamentos que possuem 
os maiores coeficientes, para posteriormente direcionarmos as nossas análises. 
No caso, nossa varíavel de interesse é quality e as variáveis com maior correlação 
são: alcohol, volatile.acidity e sulphates.
...
a variável quality e segmentando a visualização por Avaliação. E podemos observar
que há uma segmentação dos vinhos por teor alcoólico: vinhos com uma avaliação 
superior tendem a ter mais alcool. E os vinhos de avaliação inferior tendem para
uma acidez maior.

------

# Reflexão
O nosso conjunto de dados possui 1599 observações e 13 variaveis, algumas delas relacionadas as propriedades químicas dos vinhos. Nossa intenção com este conjunto 
de dados é investigar quais propriedades químicas influenciam a qualidade dos vinhos tintos.
...
também podem refletir nesta avaliação, pois alguns vinhos com as notas mais altas 
possuem teor alcoólico e a acidez próximos aos vinhos de avaliação média.
...
Outro estudo que pretendo realizar é  a investigação se os fatores que influenciam
a nota de qualidade de um vinho tinto são os mesmos para um vinho branco.
